          ...
          <div className="flex justify-center gap-4 mb-12">
            {SOCIAL_LINKS.map((link) => (
              <motion.a
                ...
                whileTap={{ scale: 0.9 }}
                className="flex items-center gap-2 px-6 py-3 rounded-xl font-semibold text-white bg-white/5 border border-white/10 hover:bg-white/10 hover:border-indigo-500/30 transition-all"
        ...
        <FadeIn delay={0.3}>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-white/[0.05]">
            <div className="text-sm text-slate-500">
              <span className="text-white font-semibold">Jonathan Vargas</span>
              {" · "}
              {PROFILE.role[lang]}
            </div>
            <div className="flex items-center gap-1 text-xs text-slate-600">
              {t(LABELS.contact.builtWith)}{" "}
              <span className="text-indigo-400 font-medium">Next.js 16</span>,{" "}
            ...
            <div className="text-sm text-slate-600">
              © {new Date().getFullYear()} Jonathan Vargas. {t(LABELS.contact.rights)}
            </div>
          </div>
        </FadeIn>
      </div>
